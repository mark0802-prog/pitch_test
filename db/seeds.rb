...
User.where("nickname like 'test%'").each do |user|
  ...
  total_time = rand(100..500)
  total_average_time = (total_time.to_f / total_correct_count).round
  ...
  daily_correct_count = rand(1...100)
  daily_wrong_count = rand(1...100)
  daily_correct_rate = (daily_correct_count.to_f / (daily_correct_count + daily_wrong_count) * 100).round
  daily_time = rand(100..500)
  daily_average_time = (daily_time.to_f / daily_correct_count).round
